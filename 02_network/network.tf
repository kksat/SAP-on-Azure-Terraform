...
resource "azurerm_virtual_network" "vnet" {
  ...
  name                = "VNET-${element(var.vnet,count.index)}"
  address_space       = ["${element(var.vnet_cidr,count.index)}"]
  location            = "${data.azurerm_resource_group.rg.location}"
  resource_group_name = "${data.azurerm_resource_group.rg.name}"
  tags                = "${merge(var.tags_map, map("Name", "Network Layer"), map("Environment", "Proof of Concept"), map("Component", "Network-Layer"))}"
}
...
resource "azurerm_network_security_group" "nsg_hub" {
  ...
  tags = "${merge(var.tags_map, map("Name", "Network Layer"), map("Environment", "Proof of Concept"))}"
}

# HUB Subnets
resource "azurerm_subnet" "subnet_hub" {
  count               = "${length(var.subnet_hub_cidrmap)}"
  name                = "SUBNET_HUB-${element(var.subnets_hub_names, count.index)}"
  resource_group_name = "${data.azurerm_resource_group.rg.name}"
...
resource "azurerm_subnet" "subnet_spoke" {
  count               = "${length(var.subnet_spoke_cidrmap)}"
  name                = "SUBNET_SPOKE-${element(var.subnets_spoke_names, count.index)}"
  resource_group_name = "${data.azurerm_resource_group.rg.name}"
